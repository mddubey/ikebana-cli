...
installer.installPackage = function (packageComponent) {
...
var downloadWithDependencies = function (packageName, packageRegistry) {
    ...
    if (installer.fs.existsSync(cachePath) && installer.fs.existsSync(packagePath)) {
        console.log('LOCAL\tCACHED\tDOWNLOADED\t' + latestPackage.name);
...
var lookupForLatestVersion = function (packageName) {
    ...
    var request = installer.http.get("http://localhost:8080/package-version.json", onResponse);
